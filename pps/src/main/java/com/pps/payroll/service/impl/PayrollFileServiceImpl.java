...
/**
 * Service to process payroll file. Acts as an abstraction for downstream
 * service class which are more specific and do actual processing Calls
 * corresponding service class which may be different for different type of
 * files
 */
@Service
public class PayrollFileServiceImpl implements PayrollFileService {
	...
	/**
	 * Processes the payroll file based on its type. If the file is a CSV, it
	 * processes the file and logs the record details. Throws exception, for
	...
	public PayrollFileResponseDTO processPayrollFile(String filePath) throws IOException, CsvException {
		...
		if (filePath.endsWith(".csv")) {
			...
			logger.info("Number of records:"
					+ (csvReaderServiceImpl.getTotalRecordsCount() - csvReaderServiceImpl.getInvalidRecords().size()));
			logger.info("Number of invalid records:" + csvReaderServiceImpl.getInvalidRecords().size());

			// Return response DTO with results
			return prepareResponse(csvReaderServiceImpl.getTotalRecordsCount(),
					csvReaderServiceImpl.getInvalidRecords().size());
		} else {

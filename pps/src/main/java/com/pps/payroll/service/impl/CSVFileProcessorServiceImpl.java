...
/**
 * Responsible for processing CSV files. Extends BaseFileProcessor which has all
 * common methods Reads CSV file, validates each record, and maps it to the
...
public class CSVFileProcessorServiceImpl extends BaseFileProcessorServiceImpl implements CSVFileProcessorService {
	...
	@Override
	/**
	 * Loads CSV file from the given path, reads all lines and then processes each
	 * record one by one
	 * 
	 * @param filePath
	 * @throws CsvException
	...
	public void processFile(String filePath) throws IOException, CsvException {
		...
		try (FileReader fileReader = readFile(filePath); CSVReader csvReader = new CSVReader(fileReader)) {
			// read all lines
	...
	/**
	 * Processes single record from the file. Validates the record and then maps it
	 * appropriate record object
	 * 
	 * @param row
	 */
	private void processCSVRecord(String[] row) {
		...
		if (recordValidator.validateRecord(row)) {
			// payroll file basically contains 2 types of records. First type if when an
			// employee gets onboarded
			// (onboard employee record). Second type is when an event happens for an
			// employee. (Employee event record)
			// An event could be bonus, exit, salary etc.
			// In the payroll file, structure of both these different record types is
			// different.
			// For example, onboard employee record has 9 fields while employee event has 6
			// fields
